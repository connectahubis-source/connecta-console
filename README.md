...
```
GitHub Repo  --push-->  GitHub Pages (静的配信)  --fetch-->  GAS WebApp (API)
                                                                    |
                                                                    v
                                                          Google Spreadsheet
```
...
```bash
git clone https://github.com/connectahubis-source/connecta-console.git
cd connecta-console
open index.html      # ブラウザで開くだけ
```

`USE_MOCK=true` の状態ではバックエンド未接続でも UI は動きます。
...
- **可搬性**: ファイル1個なので環境を選ばず動く
- **学習コスト**: ビルドツール (Vite/Webpack 等) の知識不要
- **検証速度**: 編集 → 保存 → リロード のサイクルが秒単位
- **GitHub Pages との相性**: ビルド工程なしで自動公開

### トレードオフ

- 初回ロード時に Babel が ~3MB DL + JSX 変換 → 数秒遅延
- 規模拡大時に保守性が低下する可能性

将来的にチーム開発体制が必要になったら Vite ベースへの移行を検討します。
...
- [GitHub Repository](https://github.com/connectahubis-source/connecta-console)
